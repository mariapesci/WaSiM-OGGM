...
def clim_adj_anom(rgi_ids, path_in_cru, path_in_user, temp_folder_user, plot=False):
    ...
    for glac in range(len(rgi_ids)):
        ...
        with Dataset(os.path.join(temp_folder_glac, 'climate_historical.nc') ,'w',format='NETCDF4') as nco: # create the netCDF file
            ...
            nco.yr_0 = ncdata_cru.attrs['yr_0']
            nco.yr_1 = ncdata_cru.attrs['yr_1']
...
def copy_file(path_src, path_dst, rgi_ids, name='mb_calib.json'):
    """
    This function can be used to copy the mb_calib.json file containing
    calibration parameters from OGGM using default values (W5E5 climate)
    to the user's new folder, only if the climate file is adjusted with anomalies!
    ...
    for glac in range(len(rgi_ids)):
        region = rgi_ids[glac].split('.')[0]
        src = glob.glob(path_src+'/'+region+'/'+region+'.00'+'/'+rgi_ids[glac]+'/'+name)[0]
        dst = glob.glob(path_dst+'/'+region+'/'+region+'.00'+'/'+rgi_ids[glac])[0]
        shutil.copy(src, dst)
...
def calib_one_param_mb(gdir, ref_period, default_param1='melt_f', default_param1_val=5, 
                       default_param2='temp_bias', default_param2_val=0.5,
                       calib_param='prcp_fac'):
    
    ref_mb = utils.get_geodetic_mb_dataframe().loc[gdir.rgi_id]
    ref_mb = ref_mb.loc[ref_mb['period'] == ref_period]
    glac_mb = float(ref_mb['dmdtda']) * 1000
    mb_period = np.arange(2000,2020,1)
    mbdf = pd.DataFrame(index = mb_period)
    h, w = gdir.get_inversion_flowline_hw()    
    
    if default_param1 == 'melt_f':
        melt_f = default_param1_val
        if default_param2 == 'temp_bias':
            temp_bias = default_param2_val
            mb_calibration_from_scalar_mb(gdir, ref_mb=glac_mb, ref_period=ref_period, 
                                  calibrate_param1=calib_param, melt_f=melt_f, temp_bias=temp_bias,
                                  overwrite_gdir=True)
        elif default_param2 == 'prcp_fac':
            prcp_fac = default_param2_val
            mb_calibration_from_scalar_mb(gdir, ref_mb=glac_mb, ref_period=ref_period, 
                                  calibrate_param1=calib_param, melt_f=melt_f, prcp_fac=prcp_fac,
                                  overwrite_gdir=True)
            
    elif default_param1 == 'temp_bias':
        temp_bias = default_param1_val
        if default_param2 == 'melt_f':
            melt_f = default_param2_val
            mb_calibration_from_scalar_mb(gdir, ref_mb=glac_mb, ref_period=ref_period, 
                                  calibrate_param1=calib_param, melt_f=melt_f, temp_bias=temp_bias,
                                  overwrite_gdir=True)
        elif default_param2 == 'prcp_fac':
            prcp_fac = default_param2_val
            mb_calibration_from_scalar_mb(gdir, ref_mb=glac_mb, ref_period=ref_period, 
                                  calibrate_param1=calib_param, temp_bias=temp_bias, prcp_fac=prcp_fac,
                                  overwrite_gdir=True)
            
    elif default_param1 == 'prcp_fac':
        prcp_fac = default_param1_val
        if default_param2 == 'melt_f':
            melt_f = default_param2_val
            mb_calibration_from_scalar_mb(gdir, ref_mb=glac_mb, ref_period=ref_period, 
                                  calibrate_param1=calib_param, melt_f=melt_f, prcp_fac=prcp_fac,
                                  overwrite_gdir=True)
        elif default_param2 == 'temp_bias':
            temp_bias = default_param2_val
            mb_calibration_from_scalar_mb(gdir, ref_mb=glac_mb, ref_period=ref_period, 
                                  calibrate_param1=calib_param, temp_bias=temp_bias, prcp_fac=prcp_fac,
                                  overwrite_gdir=True)
    ...
    mbdf['mod_mb'] = mb.get_specific_mb(h, w, year=mbdf.index)
    mb_params = gdir.read_json('mb_calib')
    print('melt_f: ' + str(mb_params['melt_f']) + '\n' +
          'temp_bias: ' + str(mb_params['temp_bias']) + '\n' +
          'prcp_fac: ' + str(mb_params['prcp_fac']))
    
    return 

def sens_param_mb(gdir, path_calib, ref_period, default_param='melt_f', test_sens_param='temp_bias', 
                  calib_param='prcp_fac', save_fig=False):
    ...
    mb_period = np.arange(2000,2020,1)
    h, w = gdir.get_inversion_flowline_hw()
    tb_cal_pf = {} # Dictionary containing defined range of temp_bias and calib prcp_fac
    tb_cal_mf = {} # Dictionary containing defined range of temp_bias and calib melt_f
    pf_cal_tb = {} # Dictionary containing defined range of prcp_fac and calib temp_bias
    pf_cal_mf = {} # Dictionary containing defined range of prcp_fac and calib melt_fac
    mf_cal_tb = {} # Dictionary containing defined range of melt_f and calib temp_bias
    mf_cal_pf = {} # Dictionary contianing defined range of melt_f and calib prcp_fac
    
    sens_mb = {} # Dictionary containing mass balance considering the above combination of param
    
    if default_param == 'melt_f':
        if calib_param == 'prcp_fac':
            for param_val in np.arange(-5, 5.5, 0.5):
                calib = mb_calibration_from_scalar_mb(gdir, calibrate_param1=calib_param,
                                                      calibrate_param2=test_sens_param, ref_mb=glac_mb,
                                                      ref_period=ref_period, temp_bias=param_val,
                                                      overwrite_gdir=True)
                tb_cal_pf[param_val] = calib[calib_param]
                mb = massbalance.MonthlyTIModel(gdir)
                sens_mb[param_val] = mb.get_specific_mb(h,w,year=mb_period).tolist()
                sens_glac = [tb_cal_pf, sens_mb]
            with open(path_calib + 'sens_glac_tb_cal_pf_' + gdir.rgi_id + '.json', 'w') as fp:
                json.dump(sens_glac, fp)
                
        elif calib_param == 'temp_bias':
            for param_val in np.arange(0.1, 10, 0.5):
                calib = mb_calibration_from_scalar_mb(gdir, calibrate_param1=calib_param,
                                                      calibrate_param2=test_sens_param, ref_mb=glac_mb,
                                                      ref_period=ref_period, prcp_fac=param_val,
                                                      overwrite_gdir=True)
                pf_cal_tb[param_val] = calib[calib_param]
                mb = massbalance.MonthlyTIModel(gdir)
                sens_mb[param_val] = mb.get_specific_mb(h,w,year=mb_period).tolist()
                sens_glac = [pf_cal_tb, sens_mb]
            with open(path_calib + 'sens_glac_pf_cal_tb_' + gdir.rgi_id + '.json', 'w') as fp:
                json.dump(sens_glac, fp)
        
        else:
            print('Parameters can only be "melt_f, temp_bias or prcp_fac"')
        
    elif default_param == 'temp_bias':
        if calib_param == 'prcp_fac':
            for param_val in np.arange(1.0, 11.0, 1.0):
                calib = mb_calibration_from_scalar_mb(gdir, calibrate_param1=calib_param,
                                                      calibrate_param2=test_sens_param, ref_mb=glac_mb,
                                                      ref_period=ref_period, melt_f=param_val,
                                                      overwrite_gdir=True)
                mf_cal_pf[param_val] = calib[calib_param]
                mb = massbalance.MonthlyTIModel(gdir)
                sens_mb[param_val] = mb.get_specific_mb(h,w,year=mb_period).tolist()
                sens_glac = [mf_cal_pf, sens_mb]
            with open(path_calib + 'sens_glac_mf_cal_pf_' + gdir.rgi_id + '.json', 'w') as fp:
                json.dump(sens_glac, fp)
                
        elif calib_param == 'melt_f':
            for param_val in np.arange(0.1, 10, 0.5):
                calib = mb_calibration_from_scalar_mb(gdir, calibrate_param1=calib_param,
                                                      calibrate_param2=test_sens_param, ref_mb=glac_mb,
                                                      ref_period=ref_period, prcp_fac=param_val,
                                                      overwrite_gdir=True)
                pf_cal_mf[param_val] = calib[calib_param]
                mb = massbalance.MonthlyTIModel(gdir)
                sens_mb[param_val] = mb.get_specific_mb(h,w,year=mb_period).tolist()
                sens_glac = [pf_cal_mf, sens_mb]
            with open(path_calib + 'sens_glac_pf_cal_mf_' + gdir.rgi_id + '.json', 'w') as fp:
                json.dump(sens_glac, fp)
        
        else:
            print('Parameters can only be "melt_f, temp_bias or prcp_fac"')

    elif default_param == 'prcp_fac':
        if calib_param == 'temp_bias':
            for param_val in np.arange(1.0, 11.0, 1.0):
                calib = mb_calibration_from_scalar_mb(gdir, calibrate_param1=calib_param,
                                                      calibrate_param2=test_sens_param, ref_mb=glac_mb,
                                                      ref_period=ref_period, melt_f=param_val,
                                                      overwrite_gdir=True)
                mf_cal_tb[param_val] = calib[calib_param]
                mb = massbalance.MonthlyTIModel(gdir)
                sens_mb[param_val] = mb.get_specific_mb(h,w,year=mb_period).tolist()
                sens_glac = [mf_cal_tb, sens_mb]
            with open(path_calib + 'sens_glac_mf_cal_tb_' + gdir.rgi_id + '.json', 'w') as fp:
                json.dump(sens_glac, fp)
            ...
            for param_val in np.arange(-5, 5.5, 0.5):
                calib = mb_calibration_from_scalar_mb(gdir, calibrate_param1=calib_param,
                                                      ...
                                                      ref_period=ref_period, temp_bias=param_val,
                                                      overwrite_gdir=True)
                tb_cal_mf[param_val] = calib[calib_param]
                mb = massbalance.MonthlyTIModel(gdir)
                sens_mb[param_val] = mb.get_specific_mb(h,w,year=mb_period).tolist()
                sens_glac = [tb_cal_mf, sens_mb]
            with open(path_calib + 'sens_glac_tb_cal_mf_' + gdir.rgi_id + '.json', 'w') as fp:
                json.dump(sens_glac, fp)
    ...
    if save_fig:
        colors_param = plt.get_cmap('viridis_r').colors[10::10]
        plt.figure()
        for j, param_val in enumerate(sens_glac[0].keys()):
            plt.plot(np.arange(2000,2020,1),
                 sens_glac[1][param_val], '-', linewidth=0.6, 
                 color=colors_param[j], label=param_val)
        plt.axhline(y = glac_mb, color = 'dimgrey', linestyle = '--', linewidth=2)    
        plt.ylabel(r'Annual mass-balance (mm yr$^{-1}$)', fontsize=14)
        plt.xlabel('Year', fontsize=14)
        plt.legend(title=test_sens_param+':', bbox_to_anchor=(1,1), fontsize=10)
        plt.xticks(np.arange(2000,2020,2));
        plt.title(gdir.rgi_id+' - Calibrated: '+calib_param, fontsize=14)  
        plt.tight_layout()
    
        name_fig = 'sens_mb_' + gdir.rgi_id + test_sens_param + '_calib_' + calib_param + '.png'
        plt.savefig(path_calib + name_fig, dpi=150)
            
    return   



def get_best_calib_param(gdir, path_calib, sens_name, obs_areas, melt_f_def_value, prcp_fac_def_value,
                         default_param='melt_f', test_sens_param='temp_bias', calib_param='prcp_fac',
                         var_name_to_calib='area_m2',
                         init_year=2003, end_year=2019, min_area= 2015,
                         save_sim_areas_csv=True):

    with open(path_calib + sens_name + gdir.rgi_id + '.json', 'r') as fp:
        sens_glac = json.load(fp)[0]
        for item in sens_glac.items():
            #print(item)
            if default_param == 'melt_f':
                melt_f = melt_f_def_value
                if test_sens_param == 'temp_bias':
                    temp_bias = float(item[0])
                    prcp_fac = item[1]
                    ...
                    mbmod = massbalance.MultipleFlowlineMassBalance(gdir, use_inversion_flowlines=True,
                                                                    melt_f=melt_f, temp_bias=temp_bias, prcp_fac=prcp_fac)
                    tasks.prepare_for_inversion(gdir),
                    tasks.mass_conservation_inversion(gdir),
                    tasks.filter_inversion_output(gdir),
                    tasks.init_present_time_glacier(gdir),
                    tasks.run_from_climate_data(gdir, mb_model=mbmod, climate_filename='climate_historical',
                                                output_filesuffix='_mf_'+str(melt_f)+'_tb_'+str(temp_bias)+'_pf_'+str(prcp_fac))

                elif test_sens_param == 'prcp_fac':
                    prcp_fac = float(item[0])
                    temp_bias = item[1]
                    workflow.execute_entity_task(tasks.apparent_mb_from_any_mb, gdir)
                    mbmod = massbalance.MultipleFlowlineMassBalance(gdir, use_inversion_flowlines=True,
                                                                    melt_f=melt_f, temp_bias=temp_bias, prcp_fac=prcp_fac)
            ...
            sim_areas = pd.DataFrame(index=np.arange(init_year+1,end_year+1,1))
            
    for f in os.listdir(gdir.dir):
                if f.startswith('model_diagnostics_'):
                    name_i = f.split('model_diagnostics_')[1]
                    sens_glac_i = os.path.join(gdir.dir, f)
                    sim_area_i = extract_time_series(sens_glac_i, var_name_to_calib)
                    sim_areas[name_i] = sim_area_i
                    if save_sim_areas_csv:
                        sim_areas.to_csv(path_calib+'sim_areas_sens_glac_'+gdir.rgi_id+'.csv')
                    # Compare simulated vs. observed areas in 2015:
                    obs_area_i = obs_areas.loc[gdir.rgi_id]['Area_'+str(min_area)+'_m2']
                    diff = sim_areas.loc[min_area] - obs_area_i
                    min_diff = diff.sort_values().iloc[0]
                    selected_param = diff.sort_values().index[0]
                    mf = float(selected_param.split('mf_')[1].split('_tb')[0])
                    tb = float(selected_param.split('tb_')[1].split('_pf')[0])
                    pf = float(selected_param.split('pf_')[1].split('.nc')[0])
             
    return mf, tb, pf, min_diff
